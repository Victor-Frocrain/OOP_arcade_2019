...
void ANibbler::readBestScores()
{
    ...
    fs.open("games/nibbler/.scores",  std::ios_base::in | std::ios_base::out);
    if (fs.fail()) {
...
int ANibbler::checkScores(const std::string &line) const
{
...
int ANibbler::takeScore(const std::string &line) const
{
    std::string strScore = "";
    int score = 0;
    size_t i = 0;
...
void ANibbler::writeNewScore() const
{
    std::ifstream fs;
    std::ofstream of;
    std::string line;
    ...
    fs.close();
    of.open("games/nibbler/.scores",  std::ios_base::in | std::ios_base::out);
    if (of.fail()) {
